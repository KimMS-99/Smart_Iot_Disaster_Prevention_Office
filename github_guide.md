...
#### 라. SSH 키 설정 (PAT 대신 사용 가능)
- **SSH(Secure Shell) 키란?**
  - GitHub와 같은 원격 저장소에 안전하게 연결하기 위한 암호화된 인증 방식입니다.
  - PAT와 마찬가지로 비밀번호를 매번 입력할 필요 없이 편리하게 인증할 수 있습니다.
  - 한 번 설정해두면 여러 저장소에 재사용할 수 있습니다.

- **SSH 키 생성 및 GitHub에 등록 방법:**
  1. **SSH 키 생성**: 터미널(Mac)이나 Git Bash(Windows)에서 다음 명령어를 입력합니다.
     ```bash
     ssh-keygen -t ed25519 -C "내 이메일 주소"
     ```
     - `Enter file in which to save the key (~/.ssh/id_ed25519):` 메시지가 나오면 Enter를 눌러 기본 경로에 저장합니다.
     - `Enter passphrase (empty for no passphrase):` 메시지가 나오면 비밀번호를 설정할 수 있습니다. (선택 사항, 설정 시 매번 비밀번호 입력 필요)
  2. **SSH 에이전트 시작 및 키 추가**:
     ```bash
     eval "$(ssh-agent -s)"
     ssh-add ~/.ssh/id_ed25519
     ```
  3. **공개 키(Public Key) 복사**: 생성된 공개 키(`id_ed25519.pub`)의 내용을 복사합니다.
     ```bash
     cat ~/.ssh/id_ed25519.pub
     ```
     - 출력된 `ssh-ed25519 AAAA...`로 시작하는 전체 내용을 복사합니다.
  4. **GitHub에 SSH 키 등록**:
     - GitHub에 로그인 후, 오른쪽 위 프로필 아이콘 클릭 -> Settings
     - 왼쪽 메뉴 -> SSH and GPG keys
     - 'New SSH key' 또는 'Add SSH key' 버튼 클릭
     - **Title**: 키의 용도를 적습니다. (예: 'My MacBook SSH Key')
     - **Key**: 복사한 공개 키 내용을 붙여넣기
     - 'Add SSH key' 버튼 클릭
     - GitHub 비밀번호를 다시 입력하여 확인합니다.

- **SSH URL 사용하기**:
  - 이제 원격 저장소의 URL을 `https://` 대신 `git@github.com:` 형태로 사용해야 합니다.
  - 기존에 `https`로 연결된 저장소라면, 다음 명령어로 URL을 변경할 수 있습니다.
    ```bash
    git remote set-url origin git@github.com:내아이디/내저장소.git
    ```
  - 이후부터는 `git push`나 `git pull` 시 비밀번호 입력 없이 SSH 키를 통해 인증됩니다.
...
-   `git reset`은 특정 커밋으로 되돌아가는 명령어입니다. 되돌리는 방식에 따라 세 가지 주요 옵션이 있습니다.
    -   **`git reset --soft <커밋ID>`**: 지정된 커밋으로 HEAD를 이동시키지만, 변경된 내용은 스테이징 영역에 그대로 남겨둡니다. 커밋만 취소하고 다시 커밋하고 싶을 때 유용합니다.
    -   **`git reset --mixed <커밋ID>` (기본값)**: 지정된 커밋으로 HEAD를 이동시키고, 스테이징 영역을 비웁니다. 하지만 변경된 내용은 작업 디렉토리에 그대로 남겨둡니다. 커밋과 스테이징을 모두 취소하고 다시 작업하고 싶을 때 사용합니다.
    -   **`git reset --hard <커밋ID>`**: **[주의!]** 지정된 커밋으로 HEAD를 이동시키고, 스테이징 영역과 작업 디렉토리의 모든 변경 내용을 해당 커밋 상태로 되돌립니다. **작업 중인 내용이 모두 사라지므로 매우 신중하게 사용해야 합니다.**

-   **예시**: 가장 최근 커밋을 취소하고 싶을 때
    ```bash
    git reset HEAD^ # 가장 최근 커밋 취소 (mixed 모드)
    git reset --hard HEAD^ # 가장 최근 커밋 취소 및 작업 내용 모두 삭제
    ```

#### 나. `git revert` (커밋 되돌리기 - 안전한 방법)

-   `git revert`는 특정 커밋에서 발생한 변경 사항을 되돌리는 새로운 커밋을 생성합니다. 기존 커밋 기록을 삭제하지 않고, 되돌린 이력을 남기기 때문에 협업 환경에서 안전하게 사용됩니다.
-   **예시**: 특정 커밋(`abcdefg`)의 변경 사항을 되돌리고 싶을 때
    ```bash
    git revert abcdefg
    ```
    - 이 명령을 실행하면 해당 커밋의 변경 사항을 되돌리는 새로운 커밋이 생성됩니다.

#### 다. `git log` (커밋 기록 확인)

-   `git log`는 현재 브랜치의 커밋 기록을 보여줍니다.
-   **주요 옵션**:
    -   `git log --oneline`: 각 커밋을 한 줄로 간략하게 보여줍니다.
    -   `git log --graph --oneline --all`: 모든 브랜치의 커밋 기록을 그래프 형태로 한 줄로 보여줍니다. 브랜치 흐름을 파악하는 데 유용합니다.
    -   `git log -p`: 각 커밋의 변경 내용(diff)까지 자세히 보여줍니다.

#### 라. `git diff` (변경 내용 확인)

-   `git diff`는 파일의 변경 내용을 비교하여 보여줍니다.
-   **주요 사용법**:
    -   `git diff`: 작업 디렉토리와 스테이징 영역의 차이(아직 `git add` 하지 않은 변경 내용)를 보여줍니다.
    -   `git diff --staged`: 스테이징 영역과 마지막 커밋 간의 차이(곧 커밋될 내용)를 보여줍니다.
    -   `git diff <커밋ID1> <커밋ID2>`: 두 커밋 간의 변경 내용을 비교합니다.
    -   `git diff <브랜치1> <브랜치2>`: 두 브랜치 간의 변경 내용을 비교합니다.

#### 마. 좋은 커밋 메시지 작성 가이드라인

-   커밋 메시지는 코드 변경의 이유와 내용을 다른 개발자(미래의 나 포함)에게 전달하는 중요한 수단입니다.
-   **일반적인 규칙**:
    1.  **제목(Subject Line)**: 50자 이내로 간결하게 작성하고, 변경의 목적을 명확히 드러냅니다. 명령형 동사로 시작합니다. (예: `Feat: 로그인 기능 추가`, `Fix: 버그 수정`, `Docs: README 업데이트`)
    2.  **본문(Body)**: 제목에서 다루지 못한 상세한 내용, 변경의 배경, 문제점, 해결 방법 등을 설명합니다. 72자마다 줄 바꿈을 권장합니다.
    3.  **빈 줄**: 제목과 본문 사이에 한 줄을 비워둡니다.

-   **예시**:
    ```
    Feat: 사용자 프로필 편집 기능 추가

    사용자가 자신의 프로필 정보를 웹 인터페이스에서 직접 편집할 수 있도록
    새로운 페이지와 API 엔드포인트를 추가했습니다.
    - 이름, 이메일, 비밀번호 변경 기능 포함
    - 입력 유효성 검사 로직 강화
    - 기존 `User` 모델에 `update_profile` 메서드 추가
    ```
...
#### 가. GitHub Actions (CI/CD)

-   **GitHub Actions란?**
    -   GitHub 저장소에서 직접 소프트웨어 개발 워크플로우를 자동화할 수 있도록 돕는 CI/CD(지속적 통합/지속적 배포) 플랫폼입니다.
    -   코드를 푸시하거나 풀 리퀘스트를 생성하는 등의 이벤트에 반응하여 자동으로 테스트 실행, 코드 빌드, 배포 등의 작업을 수행할 수 있습니다.
-   **주요 사용 예시**:
    -   **자동화된 테스트**: 코드를 푸시할 때마다 자동으로 테스트를 실행하여 버그를 조기에 발견합니다.
    -   **코드 빌드 및 배포**: 새로운 버전의 코드가 병합될 때마다 자동으로 애플리케이션을 빌드하고 서버에 배포합니다.
    -   **정적 분석 및 린팅**: 코드 스타일 가이드를 준수하는지 자동으로 검사합니다.
-   **작동 방식**:
    -   `.github/workflows` 디렉토리 안에 YAML 파일을 생성하여 워크플로우를 정의합니다.
    -   각 워크플로우는 하나 이상의 '잡(Job)'으로 구성되며, 각 잡은 여러 '스텝(Step)'으로 이루어집니다.
    -   스텝은 특정 명령어를 실행하거나 '액션(Action)'을 사용합니다.

#### 나. Git Stash (작업 임시 저장)

-   **`git stash`란?**
    -   현재 작업 중인 변경사항(수정된 파일, 스테이징된 파일)을 커밋하지 않고 임시로 저장하여 작업 디렉토리를 깨끗하게 만드는 명령어입니다.
    -   갑자기 다른 브랜치로 이동하여 긴급한 작업을 해야 하거나, 현재 작업 내용을 잠시 보류하고 싶을 때 유용합니다.
-   **주요 사용법**:
    -   **`git stash save "메시지"`**: 현재 변경사항을 스택에 저장하고 작업 디렉토리를 깨끗하게 만듭니다. 메시지를 추가하여 어떤 작업인지 기록할 수 있습니다.
    -   **`git stash list`**: 저장된 스택 목록을 보여줍니다.
    -   **`git stash pop`**: 가장 최근에 저장된 스택을 적용하고 스택에서 제거합니다.
    -   **`git stash apply`**: 가장 최근에 저장된 스택을 적용하지만 스택에서는 제거하지 않습니다. (여러 번 적용하고 싶을 때)
    -   **`git stash drop`**: 가장 최근에 저장된 스택을 스택에서 제거합니다.
    -   **`git stash clear`**: 모든 스택을 제거합니다.
-   **예시**:
    ```bash
    # 작업 중인 내용을 임시 저장
    git stash save "feat: login page in progress"

    # 다른 브랜치로 이동하여 긴급 버그 수정
    git switch hotfix/bug-123
    # ... 버그 수정 및 커밋 ...
    git push

    # 다시 원래 브랜치로 돌아와 임시 저장했던 내용 불러오기
